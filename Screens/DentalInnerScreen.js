...
const DentalInnerScreen = ({ navigation }) => {
    ...
    const getSurgeryList = async () => {
        ...
        let newArr = await data.filter((item) => { return item.category == route.params.id && item.dental_location == locationId })
                                ...
                                <Card key={item.id} style={{ margin: 10, backgroundColor: "white" }}  >
                                    ...
                                    <View style={{ flexDirection: "row", width: "100%" }}>
                                        ...
                                        <View style={{ marginLeft: 5, flexWrap: "wrap", marginTop: 20 }}>
                                            ...
                                            <Card.Content>
                                                <Text  allowFontScaling={false} style={{ fontWeight: "bold",fontSize:10 }}>{decode(item.hospital_name)}</Text>
